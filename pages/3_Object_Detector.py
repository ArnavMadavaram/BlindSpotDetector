...
st.set_page_config(
    page_title="ClearSight — Detector",
    layout="centered",
    page_icon="🔍",
...
.cs-tagline { color: #8b949e; font-size: 15px; margin: 8px 0 0 0; }
...
.metric-value { font-size: 30px; font-weight: 700; color: #00ffcc; line-height: 1; }
.metric-label { font-size: 11px; color: #8b949e; text-transform: uppercase; letter-spacing: 0.6px; margin-top: 6px; }

.section-heading {
    font-size: 12px; font-weight: 600; color: #8b949e;
    text-transform: uppercase; letter-spacing: 0.8px; margin: 24px 0 8px 0;
}

.stButton > button {
    background: #161b22 !important; border: 1px solid #00ffcc !important;
    color: #00ffcc !important; border-radius: 8px !important;
    font-weight: 600 !important; font-size: 15px !important;
    width: 100% !important; padding: 10px 0 !important;
    transition: all 0.15s ease !important;
}
.stButton > button:hover { background: #00ffcc !important; color: #0d1117 !important; }

.cs-footer {
    text-align: center; color: #484f58; font-size: 12px;
    padding: 32px 0 8px 0; border-top: 1px solid #21262d; margin-top: 48px;
}
</style>
...
def draw_boxes(img_rgb, detections, names):
    ...
    for det in detections:
        ...
        cv2.putText(out, label, (x1 + 4, y1 - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.58, (0, 0, 0), 2)
    return cv2.cvtColor(out, cv2.COLOR_BGR2RGB)


uploaded = st.file_uploader(
    "Drop a driving image to analyze", type=["jpg", "jpeg", "png"], label_visibility="collapsed"
)
